...
class PrepareData:
    ...
    def get_categories(cls):
        ...
        cleaned_categories = []
        min_product = 200
        # minimum number of product contained in the category
        max_product = 210
        # maximum number of product contained in the category
        ...
        for key in cls.result:
            if key == "count":
                print(f'Récupération terminée. {cls.result["count"]}'
                      f' catégories récupérées.\n')

        # loop on the categories to get those that contains only the number
        # of product defined in the min/max_product
        for categories in cls.result["tags"]:
            if min_product <= categories["products"] <= max_product:
                cleaned_categories.append((categories["name"],
                                           categories["url"]))

        # set the local list to class instance
        cls.cleaned_categories = cleaned_categories
        print("veuillez patienter lors du tri des données, celui-ci peut "
    ...
    def get_and_sort_products(cls):
        ...
        # counter to change the url in the instantiated_categories
        page_nbr = 1
        # counter to go forward in the pages of the products
        del_el = 0
        # elements ignored because of a missing key corresponding to a
        # needed data
        cat_nbr = 0
        # counter to display the final number of products contained in
        # each category
        uncleaned_products = []
        empty_slot = 0
        # counter of dat containing an empty slot (these data are
        # are ignored)

        # loop on the list of categories
        for cat in cls.cleaned_categories:
            temp_prod = []
            # temporary list of product for the current category
            ...
            if len(result["products"]) > 0:
                ...
                # append the product that contains all the needed data and
                # ignore the rest
                for prod in result["products"]:
                    try:
                        temp_prod.append((prod["product_name_fr"],
                                          prod["url"],
                                          prod["image_url"],
                                          prod["brands"],
                                          prod["stores"],
            ...
            # append the list with raw products
            position_in_cat_list += 1
            # increment the counter the get the url

        # print the number of products got in the current category
        # (the name of the category is the argument 0)
        for each in uncleaned_products:
            # increment the counter of categories
            cat_nbr += 1

        # add in the list the raw products that contains all the needed
        # data and ignore the rest
        for my_list in uncleaned_products:
            for my_product in my_list:

                if my_product[0] == "" or my_product[1] == "" \
                        or my_product[2] == "" \
                        or my_product[3] == "" \
                        or my_product[4] == "" \
                        or my_product[5] == "" \
    ...
    def calibrate(cls, list_of_data):
        """
             calibrate the data with lower case, strip and replace methods
             to make them ready to compare and eliminate
             multiple occurrences
        """
        temp_list = []

        # lower case the list
        lowercase_items = [(arg1.lower(), arg2.lower(), arg3.lower(),
                            arg4.lower(), arg5.lower(), arg6.lower(),
                            arg7.lower())
                           for arg1, arg2, arg3, arg4, arg5, arg6, arg7
                           in list_of_data]

        # strip the lower cased list
        stripped_items = [(arg1.strip(), arg2.strip(), arg3.strip(),
                           arg4.strip(), arg5.strip(), arg6.strip(),
                           arg7.strip())
                          for arg1, arg2, arg3, arg4, arg5, arg6, arg7
                          in lowercase_items]

        # replace dots in the striped list
        dot_replaced_items = [(arg1.replace(".", ""), arg2, arg3,
                               arg4.replace(".", ""), arg5.replace(".", ""),
                               arg6.replace(".", ""), arg7.replace(".", ""))
                              for arg1, arg2, arg3, arg4, arg5, arg6, arg7
                              in stripped_items]

        # slip the quotation marks in the dot replaced list
        slip_quotation_marks = [(arg1.replace("'", "''"), arg2, arg3,
                                 arg4.replace("'", "''"),
                                 arg5.replace("'", "''"),
                                 arg6.replace("'", "''"),
                                 arg7.replace("'", "''"))
                                for arg1, arg2, arg3, arg4, arg5, arg6, arg7
                                in dot_replaced_items]
    ...
    def get_url(cls, list_of_names):
        ...
        # compare the names in replaced_list with the list of categories in
        # the json file to get the corresponding url
