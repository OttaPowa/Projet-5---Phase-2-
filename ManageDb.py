...
class ManageDb:
    ...
    # store the current user_name
    user_id = []
    # store the current user_id
    list_of_user_id = []
    # list of the ids of all the users (to verify if the current user
    # already exists)

    # main variables used by all the program
    glob = ""
    # variable to locate where we are in the process of application
    action = ""
    # input of the user

    # variables used in different methods
    list_of_id = []
    # list of the product ids (used to check if the tipped id is inside
    ...
    base_product = ""
    # id of the original product chosen by the user
    alternative_product = ""
    # id of the final product chosen by the user
    original_and_alternative_prod_ids = []
    # list of the ids of the base product and the final product

    @staticmethod
    def verify_prerequisite():
        """
            verify if MySQL is installed on the client computer. if not print
             a message with à link to DL it.
        """

        print("\nBonjour, MySQL est nécéssaire pour faire fonctionner "
              "l'application.\nVous pouvez l'installer en suivant ce "
              "lien: https://dev.mysql.com/downloads/mysql/#downloads\n")
    ...
    def mysql_connection(cls):
        ...
        try:
            cls.connexion = mysql.connector.connect(user=user_name,
                                                    password=password)
    ...
    def ask_to_update_database(cls):
        ...
        question = input("\nVoulez mettre à jour la base de donnée? "
                         "ATTENTION: ceci supprimera vos sauvegardes!\n"
                         "(Y): oui, (N): non: ")
    ...
    def access_to_stored_data(cls):
        ...
        if question == "Y" or question == "y":
            base_prod = f"saving " \
                        f"INNER JOIN product AS product1 " \
                        f"ON product1.id = saving.base_product_id " \
                        f"INNER JOIN product AS product2 " \
                        f"ON product2.id = saving.alternative_product_id " \
                        f"WHERE saving.user_id = {user_id}"
            result_bp = cls.select(("product1.id",
                                    "product1.name",
                                    "product1.nutriscore",
                                    "product2.id",
                                    "product2.name",
                                    "product2.nutriscore"), base_prod)

            if not result_bp:
                print("Aucune donnée sauvegardé n'a été trouvé pour votre"
                      " utilisateur")
            else:
                print("\nVous aviez trouvé :")
                for index, result in enumerate(result_bp):
                    print(f"    {index + 1}) {result[4]} au nutriscore de "
                          f"{result[5].capitalize()} comme alternative à "
                          f"{result[1]} et son nutriscore de "
                          f"{result[2].capitalize()}")
    ...
    def build(cls):
        ...
        data_base_sql = """
                        ...
                        ALTER TABLE product_category 
                        ADD CONSTRAINT product_category_FK_1 
                        ...
                        ALTER TABLE product_brand 
                        ADD CONSTRAINT product_brand_FK_1 
                            FOREIGN KEY (id_product) REFERENCES product(id);
                        ...
                        ALTER TABLE product_store 
                        ADD CONSTRAINT product_store_FK_1 
                            FOREIGN KEY (id_product) REFERENCES product(id);
    ...
    def insert_logs_and_get_id(cls):
        ...
        for item in User.instantiated_logs:
            cls.cursor.execute("INSERT IGNORE INTO user (user_name, password)"
                               " VALUES (%s, %s)", (item.user_name,
                                                    item.password))
        # get the id
        cls.user_id = cls.select(("id",), f'user WHERE user_name = '
                                          f'"{cls.my_current_user_name}"')
    ...
    @classmethod
    def insert_n_n(cls, instantiated_list, name_of_table1, name_of_table2,
                   name_of_table3, column1, column2):
        """
            Prepare and insert data in the n_n tables
        """

        cat_unfounded = []  # categories not found in the french Api category
        ...
        for product in instantiated_list:
            ...
            query_product_id = f'SELECT id FROM {name_of_table1} ' \
                               f'WHERE name = "{product.name}"'
            ...
            # do the associations of item id with the products id of the list
            for my_prod_id in my_list_of_product_id:
                ...
                # select the id of the store, brand or category where the
                # name is the "item" value
                for item in cleaned_list_of_items:
                    query_item_id = f'SELECT id FROM {name_of_table2} ' \
                                    f'WHERE name = "{item}"'
                    ...
                    # some index error occurs when a name in another language
                    # is found (english or german mostly)
                    try:
                        query_insert = f"INSERT IGNORE INTO " \
                                       f"{name_of_table3} (" \
                                       f"{column1}, {column2})" \
                                       f" VALUES (%s, %s)"
                        cls.cursor.execute(query_insert, (my_prod_id[0],
                                                          my_item_id[0]))
    ...
    def display_products(cls, my_category):
        ...
        # get the product id, name and nutriscore depending of the
        # category id
        my_products_query = f"product INNER JOIN product_category " \
                            f"ON product.id = product_category.id_product " \
                            f"INNER JOIN category " \
                            f"ON category.id = " \
                            f"product_category.id_category " \
                            f"WHERE category.id = {my_category}"
        prods_id_name_nutriscore = cls.select(("product.id", "product.name",
                                               "product.nutriscore"),
                                              my_products_query)

        # get the category name
        cat_name = cls.select(("category.name",), f"category "
                                                  f"WHERE category.id = "
                                                  f"{my_category}")
        print(f"\nVoici les produits de la catégorie "
              f"{cat_name[0][0].capitalize()}:\n")
    ...
    def display_nutriscore(cls, user_action):
        ...
        if int(user_action) in cls.list_of_id:
            ...
            for my_product in result:
                ...
                print(f'\nLe nutriscore de {my_product[1]} est '
                      f'{my_product[4].capitalize()}')

        else:
            print("Ce chiffre ne correspond pas a un produit de la catégorie "
                  "que vous avez choisis")

    @classmethod
    def compare_product_in_current_category(cls):
        """
            compare the product with others from his category, display those
            that have the higher nutriscore
        """

        current_nutriscore = cls.current_product[0][4].capitalize()
        # set the current nutriscore
        current_id = cls.current_product[0][0]  # set the current id

        if current_nutriscore == "A" or current_nutriscore == "a":
            print('\nVous avez déjà un produit qui a un nutriscore de A,'
                  ' impossible de vous proposer quelque-chose de '
                  'mieux!')

        else:
            # get the products id, name and nutriscore in the current category
            query = f"product INNER JOIN product_category " \
                    f"ON product.id = product_category.id_product " \
                   f"INNER JOIN category on product_category.id_category =" \
                    f" category.id " \
                   f"WHERE category.id = {cls.my_category_id} " \
                    f"ORDER BY nutriscore"
            prods_from_mother_cat = cls.select(("product.id",
                                                "product.name",
                                                "product.nutriscore"), query)

            # display the product with the higher nutriscore til reach the
            # nutriscore of the current product
            for my_product in prods_from_mother_cat:
                if my_product[2].capitalize() < current_nutriscore \
                        and my_product[0] != current_id:
                    print(f"\nNous vous proposons "
                          f"{my_product[1].capitalize()} comme produit de "
                          f"substitution avec son Nutriscore de "
                          f"{my_product[2].capitalize()}.")

                    # query the user. invoke the next method in line and break
                    # the loop
                    choice = input("\nVoulez vous voir les détails de ce "
                                   "produit?\n(Y): Oui, (N): Non: ")
    ...
    def compare_product_in_affiliated_categories(cls, product_id):
        """
            get and display the products with a nutriscore of A in
            affiliated categories of the chosen product
        """

        print("\nNous allons maintenant rechercher un produit dans "
              "les catégories affiliées au produit...")

        # get the categories ids affiliated to the product_id
        query_cat_id = f"category INNER JOIN product_category " \
                       f"ON category.id = product_category.id_category " \
        ...
        for my_cat_id in categories_ids:
            ...
            choose_prod = input("\nVoulez vous explorer un produit de "
                                "cette liste?\n"
                                "(Oui: tapez le numéro du produit), "
                                "(Entrée: Continuer vers une autre catégorie)"
                                ", (Q: Quitter): ")

            # break the loop and launch class method display_nutriscore if
            # the user enter a product number
        ...
        print("\nDésolé mais vous avez épuisé tous les produits de toutes "
              "les  catégories affiliées...\n")
    ...
    def show_final_product_details(cls, product_id):
        ...
        print(f"\nVous trouverez une fiche détaillée sur "
              f"{product_details[0][1].capitalize()} en suivant le lien:\n"
              f"  - {product_details[0][2]}")

        # get the stores of the product
        query_store = f"store INNER JOIN product_store " \
                      f"ON store.id = product_store.id_store " \
                      f"INNER JOIN product " \
                      f"ON product.id = product_store.id_product " \
        ...
        query_brand = f"brand INNER JOIN product_brand " \
                      f"ON brand.id = product_brand.id_brand " \
                      f"INNER JOIN product " \
                      f"ON product.id = product_brand.id_product " \
                      f"WHERE product.id = {product_id}"
    ...
    def ask_to_save_result(cls):
        ...
        if cls.glob != "save search":
            choice = input("\nVoulez vous enregister ce résultat de "
                           "recherche?\n"
                           "(Y): oui, (Entrée): continuer, (Q): quitter:  ")

            # store the base product id and the final product id to
            # instantiate later
            if choice == "Y" or choice == "y":
                cls.original_and_alternative_prod_ids = \
                    [(cls.base_product, cls.alternative_product)]
                cls.save_search()
    ...
    def save_search(cls):
        """
            save the base product id and the alternative product id
            into the table saving. Then link tables user and saving
            into user_saving to associate user and data.
        """

        print("\nSauvegarde de votre recherche...")

        # instantiate base_product_id and alternative_product_id into
        # class Saving
        PrepareData.instantiate(Saving,
                                cls.original_and_alternative_prod_ids)

        # insert instantiated data into the saving table
        for item in Saving.instantiated_saving:
            cls.cursor.execute("INSERT INTO saving "
                               "(base_product_id, alternative_product_id,"
                               " user_id) "
                               "VALUES (%s, %s, %s)",
        ...
        cls.connexion.commit()
        print("\nVos produit ont bien été enregistrés.")
    ...
    def selection(cls):
        ...
        if cls.glob == "":
            ...
            value = "Tapez le numéro de la catégorie que vous souhaitez" \
                    " explorer vous pouvez quitter en tapant (Q): "
        elif cls.glob == "display prod":
            value = "Tapez le numéro du produit que vous souhaitez " \
                    "explorer\n" \
                    "vous pouvez quitter en tapant (Q) et revenir aux" \
                    " catégories en tapant (C): "
        elif cls.glob == "compare prod":
            value = "Voulez-vous chercher un produit similaire avec un" \
                    " meilleur nutriscore?\n" \
                    "(Y): oui, (C): retour aux catégories, (Q): quitter: "
        elif cls.glob == "alternative compare":
            value = "Effectuer une recherche dans les catégories affiliées" \
                    " à votre produit à substituer ?\n" \
                    "(Y): oui, (S): ce produit me convient, (C): retour" \
                    " aux catégories, (Q): quitter: "
        elif cls.glob == "show details":
            value = "Voulez vous voir les détail d'un produit?\n" \
                    "(tapez le numéro du produit): Oui, (C): retour" \
                    " aux catégories (Q): Quitter: "
        elif cls.glob == "save search":
            value = "Voulez vous sauvegarder ce produit et les données" \
                    " s'y rapportant?\n" \
                    "(Y): Oui, (C): retour aux catégories, (Q): quitter: "
